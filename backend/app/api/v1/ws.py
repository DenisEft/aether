...
async def widget_websocket(
    ...
    if not await ws_manager.register(
        conn_id, websocket, channel_type, str(tenant_id), user_id, str(tenant_id), session_id
    ):
        await websocket.close(
            code=status.WS_1011_INTERNAL_ERROR, reason="Failed to register connection"
        )
        return
    ...
    try:
        connected_msg = {
            ...
            "config": {
                ...
                "rate_limit": {"messages_per_minute": 30, "messages_per_day": 1000},
                "working_hours": {"enabled": True, "timezone": "UTC", "schedule": "09:00-18:00"},
            },
        }
...
async def _handle_inbound(
    websocket: WebSocket, conn_id: str, user_id: str, tenant_id: uuid.UUID, raw: str, last_seq: int
):
...
async def _handle_chat_message(
    websocket: WebSocket, conn_id: str, user_id: str, tenant_id: uuid.UUID, msg: dict, seq: int
):
    ...
    metadata.get("channel_type") if metadata else "widget"
...
async def _generate_ai_response(
    ...
    try:
        ...
        logger.exception(
            "AI response generation failed for tenant=%s conv=%s", tenant_id, conversation_id
        )
        error_msg = {
            "type": "system.error",
            "id": f"err_{uuid.uuid4().hex[:8]}",
