...
class SmartRouter:
    ...
    async def route(
        self, request: InferenceRequest, strategy: RoutingStrategy | None = None
    ) -> RoutingResult:
        """
        ...
        return RoutingResult(
            driver=best_driver, score=best_score, strategy=strategy, fallback_chain=fallback_chain
        )

    async def _get_available_drivers(
        self, model_id: str | None, strategy: RoutingStrategy
    ) -> list[BaseDriver]:
        ...
        for driver in self.pool.get_all_drivers():
            ...
            try:
                if model_id and model_id in await driver.get_available_models() or not model_id:
    ...
    async def _score_driver(
        self, driver: BaseDriver, request: InferenceRequest, strategy: RoutingStrategy
    ) -> float:
        """
        ...
        try:
            ...
            # Get scoring weights
            weights = self._scoring_weights.get(
                strategy, self._scoring_weights[RoutingStrategy.HYBRID]
            )
            ...
            if model_info:
                ...
                quality_score = (
                    model_info.quality_score if hasattr(model_info, "quality_score") else 1.0
                )

            # Latency score (lower is better)
            if health.latency_ms > 0:
                latency_score = 1.0 / (
                    1.0 + health.latency_ms / 1000.0
                )  # Normalized by 1000ms threshold
            else:
        ...
        if request.tenant_id is not None:
            ...
            if callback:
                try:
                    ...
                    logger.warning(
                        f"Billing callback failed for streaming tenant {request.tenant_id}: {e}"
                    )
