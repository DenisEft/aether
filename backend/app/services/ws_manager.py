...
class WebSocketManager:
    ...
    def __init__(self):
        ...
        self.connections: dict[str, dict[str, dict[str, dict]]] = defaultdict(
    ...
    async def register(
        ...
        try:
            ...
            logger.info(
                "WebSocket registered: connection_id=%s, channel=%s/%s",
                connection_id,
                channel_type,
                channel_id,
            )
        ...
        try:
            ...
            for conn_id, connection_info in list(connections.items()):
                try:
                    ...
                    if ws and ws.application_state != "DISCONNECTED":
            ...
            logger.debug(
                "Broadcast sent to %d connections in %s/%s", sent_count, channel_type, channel_id
            )
            return sent_count
    ...
    async def send_personal(self, message: dict, connection_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to send personal message to %s: %s", connection_id, e, exc_info=True
            )
            return False
    ...
    async def update_heartbeat(self, connection_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to update heartbeat for connection %s: %s", connection_id, e, exc_info=True
            )
            return False
    ...
    async def send_read_receipt(self, channel_type: str, channel_id: str, message_id: str) -> int:
