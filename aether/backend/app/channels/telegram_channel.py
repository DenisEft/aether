...
from .base import BaseChannel, ChannelConfig, ChannelMessage, ChannelStatus, MessageContext
...
class TelegramChannel(BaseChannel):
    ...
    async def send_message(self, chat_id: str, text: str, parse_mode: str = "HTML", reply_markup: dict = None) -> dict:
    ...
    async def send_photo(self, chat_id: str, photo_url: str, caption: str = "") -> dict:
        return await self._call("sendPhoto", {"chat_id": chat_id, "photo": photo_url, "caption": caption})
    
    async def send_file(self, chat_id: str, file_url: str, filename: str = "") -> dict:
        return await self._call("sendDocument", {"chat_id": chat_id, "document": file_url, "caption": filename})
    
    async def send_quick_replies(self, chat_id: str, text: str, buttons: list[str]) -> dict:
        keyboard = [[{"text": btn}] for btn in buttons]
        return await self.send_message(chat_id, text, reply_markup={"keyboard": keyboard, "resize_keyboard": True, "one_time_keyboard": True})
    
    async def set_webhook(self, url: str) -> bool:
        result = await self._call("setWebhook", {"url": url, "allowed_updates": ["message", "callback_query"]})
        if result:
    ...
    async def get_updates(self, limit: int = 100, timeout: int = 30) -> list[dict]:
        updates = await self._call("getUpdates", {"offset": self._offset, "limit": limit, "timeout": timeout})
        if updates:
            ...
            try:
                ...
                for update in updates:
                    ...
                    ctx = MessageContext(
                        ...
                        external_user_name=f"{user.get('first_name', '')} {user.get('last_name', '')}".strip() or user.get("username", "Unknown"),
    ...
    async def listen_webhook(self, payload: dict) -> Optional[MessageContext]:
